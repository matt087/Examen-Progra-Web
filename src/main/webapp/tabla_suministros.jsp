...
<html>
	<head>
		...
		<title>Suministros Fundación</title>
	</head>
	<body>
		...
		<main class='centrar_vertical'>
			<%
					int id_fund = Integer.valueOf(request.getParameter("cmbFundaciones"));
					sesion.setAttribute("fundacion", id_fund);
					Fundacion fun = new Fundacion();
					String nombre = fun.getNombreById(id_fund);
					out.print("<h2>Suministros de la Fundación "+nombre+"</h2>");
					out.print(fun.crearTablaSuministros(id_fund));

			%>
			<div class='registro_fun'>
				<a href='añadir_suministros.jsp'><i class="bi bi-patch-plus-fill"></i>Añadir Suministros</a>
			</div>
		</main>
		<footer>
